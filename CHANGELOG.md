...
- **#890 — the oracle-wiring gate: fix the factory, not the bugs.**
  ...
  workflow references it, when a `manual`/`unwired` declaration has no real
  reason or an out-of-vocabulary category, or when a script declares nothing at
  all. New oracles are forced to choose. Rationale, the categorized manual set,
  and the red-first transcript: `scripts/repro/ORACLE_WIRING.md`.
