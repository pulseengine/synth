...
See `coq/STATUS.md` for the complete coverage matrix. Current: 622 Qed / 2 Admitted
(+2 `admit.` tactics) across `coq/Synth/`. The 79 selector-DSL rule theorems
...
- **Track A (core):** `VCR-RA-001` allocator with Belady spilling — **verified,
  ...
  verified selector DSL (increments 1–6 shipped **default-on**, 79 rules / 79 Qed;
  the Rocq-proved rules are the ONLY lowering path for their 59 covered ops —
...
- **Track B (semantics):** `VCR-ISA-001` Sail-generated Rocq ISA model —
  ...
  source, the 79 correctness Qed are stated directly about it, and a
