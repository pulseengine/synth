...
let eval_condition cond f =
  ...
  | Cond_GE -> eqb f.flag_n f.flag_v
  | Cond_LT -> negb (eqb f.flag_n f.flag_v)
  | Cond_GT -> (&&) (negb f.flag_z) (eqb f.flag_n f.flag_v)
  | Cond_LE -> (||) f.flag_z (negb (eqb f.flag_n f.flag_v))
  | Cond_AL -> true
