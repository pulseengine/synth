...
|-------|------------------|----------------|
| Rocq proof suite | 536 Qed / 3 Admitted (+2 `admit.` tactics) | T1 result-correspondence for all i32 and i64 selection; T2 existence-only for float/SIMD; trusted base: 93 Axiom/Parameter declarations |
...
| WasmCert-Coq source anchor (VCR-WASM-001) | 49 Qed | `coq/Synth/WASM/WasmCertBridge.v` — i32 integer fragment refined against pinned WasmCert-Coq rules |
