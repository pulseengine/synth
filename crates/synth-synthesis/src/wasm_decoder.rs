            ...
            Payload::DataSection(reader) => {
                for data in reader {
                    ...
                    if let wasmparser::DataKind::Active { memory_index: 0, offset_expr } = data.kind {
                        // Parse the offset expression (should be i32.const)
...
pub fn decode_wasm_functions(wasm_bytes: &[u8]) -> Result<Vec<FunctionOps>> {
    ...
    // Map from function index to export name(s)
    let mut export_names: HashMap<u32, String> = HashMap::new();
    ...
    for payload in Parser::new(0).parse_all(wasm_bytes) {
        ...
        match payload {
            ...
            // Collect function exports (name -> function index)
            Payload::ExportSection(exports) => {
                for export in exports {
                    let export = export.context("Failed to parse export")?;
                    if export.kind == ExternalKind::Func {
            ...
            Payload::CodeSectionEntry(body) => {
                let ops = decode_function_body(&body)?;
                // Actual function index = imported functions + local index
                let actual_index = num_imported_funcs + func_index;
                let export_name = export_names.get(&actual_index).cloned();
...
fn convert_operator(op: &wasmparser::Operator) -> Option<WasmOp> {
    ...
    match op {
        ...
        CallIndirect { type_index, table_index, .. } => Some(WasmOp::CallIndirect {
...
mod tests {
    ...
    fn test_decode_simple_add() {
        ...
        // Note: End is now included in decoded output for control flow support
        assert_eq!(
            functions[0].ops,
            vec![WasmOp::LocalGet(0), WasmOp::LocalGet(1), WasmOp::I32Add, WasmOp::End]
        );
    }
    ...
    fn test_decode_multi_function_module() {
        // Multi-function module like in official spec tests
        let wat = r#"
            (module
