...
impl InstructionSelector {
    ...
    pub(super) fn select_default(&mut self, wasm_op: &WasmOp) -> Result<Vec<ArmOp>> {
        ...
        let instrs = match wasm_op {
            ...
            //
            // RQ-69-SUBTRACT (#242): the Helium/MVE lowering path that used to
            // sit here — 88 `if self.has_helium` arms — was DELETED. v0.68
            // measured it production-dead by construction, not merely
            // uncovered: `set_helium(true)` occurs only inside `mod tests`, and
            // the decoder refuses every SIMD operator on every target (a v128
            // module is refused at decode on cortex-m55 exactly as on
            // cortex-m4). There was no target on which those arms ran.
            //
            // The arm below already handled every one of the same 91 ops when
            // Helium was absent, which in production was always — so deleting
            // Path A removes no decline and changes no emitted byte. That is
            // the gate this deletion passed: byte identity over the corpus.
            //
            // Restoring MVE lowering means restoring it from git history AND
            // wiring `set_helium` to a real target capability AND teaching the
            // decoder to accept v128 — all three, or it is dead again.
