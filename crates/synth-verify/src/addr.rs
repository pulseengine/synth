...
//! retargeting the compiler actually emitted — one
//! [`RelocResolution`] `(seg_index K, addend A)` per static-data reloc — it
//! reconstructs the RUNTIME linear-memory image (apply every segment in
//! declaration order, later-wins) **independently of K**, then asserts: the
//! byte the packed `.data` serves for that reloc
//! (`seg[K].bytes[A]`) EQUALS the byte the runtime image holds at the reloc's
//! original access address (`seg[K].off + A`). A single mismatch — the
//! wrong-segment resolution — is a [`Verdict::Mismatch`].
//!
//! # Concrete, not symbolic
//!
//! This is a concrete byte-equality over a compiled object, not a ∀-inputs
//! SMT property: there are no symbolic operands (unlike [`crate::trap`]). It
//! mirrors VCR-VER-002's *structure* (a verdict enum + a per-compilation gate
//! wired default-on) but does the honest thing — a direct comparison against
//! an independently reconstructed truth image. The truth side never touches
//! `K`, so the validator cannot be satisfied by mirroring the code under test
//! (the VCR-ORACLE mirror-pinning failure mode is structurally excluded).
...
fn runtime_image(segments: &[DataSegment]) -> std::collections::HashMap<u32, u8> {
...
mod tests {
    ...
    /// so this is a permanent test, not a one-time source revert.
    #[test]
    fn red_on_first_match_green_on_last_match() {
        let segs = overlapping_segments();
        // Access the byte at linmem 0x100008 — the classic #757 access (the
        // string's 9th byte). All three segments cover it with distinct bytes.
        let c = 0x100008;
